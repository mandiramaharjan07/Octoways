...
import { Waypoint } from '../types/otto';
...
const WAYPOINTS = (vw: number, vh: number): Waypoint[] => [
  { 
    id: "entrance",
    x: vw + 100, y: vh * 0.6,
    enterFrom: "right",
...
export default function useOttoPath() {
  ...
  const [dimensions, setDimensions] = useState<Dimensions>({ vw: window.innerWidth, vh: window.innerHeight });
  const [isGuiding, setIsGuiding] = useState<boolean>(false);
  const x = useMotionValue<number>(dimensions.vw + 100);
  const y = useMotionValue<number>(dimensions.vh * 0.6);
  ...
  useEffect(() => {
    ...
    let targetX = activeWaypoint.x || 0;
    let targetY = activeWaypoint.y || 0;
    ...
    const controls = animate(x, targetX, {
      duration: (activeWaypoint.duration || 0) / 1000,
      ease: "easeOut"
    });
  ...
  const highlightSequence = useCallback(async (steps: HighlightStep[]) => {
    setIsGuiding(true);
