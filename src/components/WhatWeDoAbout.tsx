...
export default function WhatWeDoAbout() {
  return (
    <section id="about-what-we-do" className="py-24 px-8 bg-animated-neural relative overflow-hidden transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="font-headline text-4xl md:text-5xl font-bold mb-6 text-text-primary">What We Do</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <motion.div
              ...
              className={`bg-surface-card p-10 rounded-[2.5rem] transition-all group shadow-card hover:shadow-xl ${index === 4 ? 'lg:col-span-1 md:col-span-2' : ''}`}
            >
              <div className="w-16 h-16 rounded-2xl bg-primary/10 flex items-center justify-center mb-8 group-hover:bg-primary/20 transition-colors shadow-sm">
                <span className="material-symbols-outlined text-4xl text-primary">{service.icon}</span>
              </div>
              <h3 className="font-headline text-2xl font-bold mb-4 text-text-primary transition-colors group-hover:text-primary">{service.title}</h3>
