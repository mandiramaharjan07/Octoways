    ...
    <section className="relative min-h-[50vh] flex items-center px-8 pt-20 overflow-hidden bg-animated-neural">
      ...
      <div className="max-w-7xl mx-auto w-full relative z-10 text-center">
        <div className="flex justify-center items-center gap-2 mb-6 reveal">
          ...
          <span className="text-primary-fixed-dim text-xs font-bold uppercase tracking-[0.3em]">Excellence in Execution</span>
        </div>
        ...
        <div className="flex justify-center gap-4 md:gap-12 reveal delay-300">
          {stats.map((stat, i) => (
            <div key={i} className="text-center px-4 md:px-8 border-x first:border-l-0 last:border-r-0 border-white/10">
              <div className="text-2xl md:text-3xl font-headline font-bold text-primary mb-1">{stat.value}</div>
              <div className="text-[10px] md:text-xs text-gray-400 uppercase tracking-widest font-bold">{stat.label}</div>
            </div>
          ))}
        </div>
      </div>
