...
const PortfolioGrid: React.FC<PortfolioGridProps> = ({ activeCategory }) => {
  const filteredProjects = activeCategory === 'All' 
    ? projects 
    : projects.filter(p => p.categories.includes(activeCategory));

  return (
    <section id="works-grid" className="py-24 px-8 bg-background transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
          ...
          <AnimatePresence mode='popLayout'>
            {filteredProjects.map((project) => (
              <motion.div
                ...
                className="bg-surface-card border border-border rounded-[2.5rem] overflow-hidden group hover:bg-surface-raised transition-all duration-500 shadow-card hover:shadow-xl hover:border-border-accent"
              >
                <div className="aspect-[4/3] overflow-hidden relative">
                  ...
                  <div className="absolute bottom-6 left-6 right-6">
                    <span className="bg-primary text-on-primary text-[10px] font-bold px-4 py-1.5 rounded-full uppercase tracking-widest mb-3 inline-block shadow-lg">
                      {project.type}
                    </span>
                    <h2 className="font-headline text-2xl font-bold text-text-primary group-hover:text-primary transition-colors duration-300">
                      {project.name}
                    </h2>
                  </div>
                </div>
                ...
                <div className="p-10">
                  <p className="text-secondary text-xs font-bold uppercase tracking-widest mb-3">{project.tagline}</p>
                  ...
                  <div className="flex items-center justify-between pt-8 border-t border-border">
                    ...
                    <button className="text-text-primary text-[11px] font-bold uppercase tracking-widest flex items-center gap-2 hover:text-primary transition-colors group/btn">
                      View Case <span className="material-symbols-outlined text-sm transition-transform group-hover/btn:translate-x-1">arrow_forward</span>
                    </button>
                  </div>
                </div>
              </motion.div>
        ...
        {filteredProjects.length === 0 && (
          <div className="text-center py-32 text-text-muted font-headline text-2xl">
